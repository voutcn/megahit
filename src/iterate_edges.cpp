...
using std::string;
using std::vector;
#define HashTable spp::sparse_hashtable
...
static bool ReadReadsAndProcessKernel(
    IterateGlobalData &globals,
    spp::sparse_hash_set<KmerPlus<kNumKmerWord_p, kmer_word_p_t, MarginalKmer>, KmerHash> &crusial_kmers) {
    if (Kmer<kNumKmerWord_n, kmer_word_n_t>::max_size() < (unsigned)globals.kmer_k + globals.step + 1) {
        return false;
    }

    spp::sparse_hash_set<KmerPlus<kNumKmerWord_n, kmer_word_n_t, uint16_t>, KmerHash> iterative_edges;
    ...
    while (true) {
        ...
        for (unsigned i = 0; i < (unsigned)rp.size(); ++i) {
            ...
            KmerPlus<kNumKmerWord_p, kmer_word_p_t, MarginalKmer> kp, rkp;
            Kmer<kNumKmerWord_p, kmer_word_p_t> &kmer = kp.kmer;
            Kmer<kNumKmerWord_p, kmer_word_p_t> &rev_kmer = rkp.kmer;
            ...
            while (cur_pos + globals.kmer_k + 1 <= length) {
                ...
                if (!kmer_exist[cur_pos]) {
                    auto iter = crusial_kmers.find(kp);
                    ...
                    else if ((iter = crusial_kmers.find(rkp)) != crusial_kmers.end()) {
                        kmer_exist[cur_pos] = true;
                        uint64_t s_seq = iter->ann.s_seq;
                        int s_seq_length = s_seq & 63;
                        float mul = iter->ann.mul;

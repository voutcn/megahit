...
class Kmer {
 public:
  ...
  Kmer() {
    std::memset(data_, 0, sizeof(word_t) * NumWords);
  }

  Kmer(const Kmer &kmer) {
    std::memcpy(data_, kmer.data_, sizeof(word_t) * NumWords);
  }

  Kmer(const word_t *seq, int offset, int k) {
    InitFromPtr(seq, offset, k);
  }

  void InitFromPtr(const word_t *seq, int offset, int k) {
    ...
    memset(data_ + used_words, 0, sizeof(word_t) * (NumWords - used_words));
  }

  ~Kmer() = default;

  const word_t *DataPtr() const {
    return data_;
  }
  ...
  void ShiftAppend(uint8_t ch, int k) {
    ...
    data_[used_words - 1] = (data_[used_words - 1] << 2)
        | (word_t(ch) << ((kCharsPerWord - 1 - (k - 1) % kCharsPerWord) << 1));
  }
  ...
  void SetBase(uint32_t index, uint8_t ch) {
    ...
    data_[index / kCharsPerWord] =
        (data_[index / kCharsPerWord] & ~(word_t(3) << offset))
            | (word_t(ch) << offset);
  }
  ...
  void clear() {
    memset(data_, 0, sizeof(word_t) * NumWords);
  }
  ...
  static const unsigned n_bytes = sizeof(word_t) * kNumWords;

 private:
  word_t data_[NumWords];
} __attribute__((packed));

namespace std {
template<const unsigned kNumWords, typename T>
inline void swap(Kmer<kNumWords, T> &kmer1, Kmer<kNumWords, T> &kmer2) {
  kmer1.swap(kmer2);
}
}

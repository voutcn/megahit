...
class Kmer {
 public:
  ...
  bool operator==(const Kmer &kmer) const { return memcmp(data_, kmer.data_, sizeof(data_)) == 0; }
  ...
  unsigned kth_byte(unsigned k) const { return (data_[k / sizeof(word_type)] >> k % sizeof(word_type) * 8) & 0xFF; }

...
ostream &operator<<(ostream &os, const OptionsDescription &desc) {
  for (unsigned i = 0; i < desc.options.size(); ++i) os << desc.options[i] << endl;

  return os;
}

ostream &operator<<(ostream &os, const OptionsDescription::Option &option) { return os << string(option); }
...
void OptionsDescription::Parse(int &argc, char *argv[]) {
  ...
  for (unsigned i = 0; i < options.size(); ++i) {
    ...
    long_options[i].val = (options[i].short_name != "" ? options[i].short_name[0] : 0);

    if (options[i].type == kOptionBool) {
      if (options[i].short_name != "") short_options += options[i].short_name;
  ...
  while (true) {
    ...
    if (ch == '?') throw logic_error("uknown option");
  ...
  for (int i = optind; i < argc; ++i) argv[index++] = argv[i];

  argc = index;

  for (unsigned i = 0; i < options.size(); ++i) options[i].Parse();
}

OptionsDescription::operator string() const {
  stringstream ss;

  for (unsigned i = 0; i < options.size(); ++i) ss << options[i] << endl;

  return ss.str();
}

void OptionsDescription::AddOption(const std::string &long_name, const std::string &short_name, bool &bool_option,
                                   const std::string &description) {
  AddOption(long_name, short_name, &bool_option, kOptionBool, description, "");
}

void OptionsDescription::AddOption(const std::string &long_name, const std::string &short_name, int &int_option,
                                   const std::string &description) {
  stringstream ss;
  ss << int_option;
...
void OptionsDescription::AddOption(const string &long_name, const string &short_name, void *pointer, OptionType type,
                                   const string &description, const string &default_value) {
  for (unsigned i = 0; i < options.size(); ++i) {
    if (options[i].long_name == long_name || (short_name != "" && options[i].short_name == short_name)) {
...
OptionsDescription::Option::Option(const string &long_name, const string &short_name, void *pointer, OptionType type,
                                   const string &description, const string &default_value) {
...
void OptionsDescription::Option::Parse() {
  ...
  if (value != "") {
    switch (type) {
      case kOptionBool:
        if (value != "" || value == "false") *(bool *)pointer = true;

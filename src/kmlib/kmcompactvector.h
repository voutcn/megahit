...
class CompactVector {
  static_assert(sizeof(WordType) * 8 % BaseSize == 0, "BaseSize must of power of 2 and no larger than TWord");
 ...
 public:
  class Adapter {
   ...
   public:
    Adapter(word_type *word, unsigned base_offset) : word_(word), bit_offset_(bit_shift(base_offset)) {}
 ...
 public:
  static size_type size_to_word_count(size_type size) { return (size + kBasesPerWord - 1) / kBasesPerWord; }
  static unsigned bit_shift(unsigned pos, unsigned len = 1) {
    return Endian == kBigEndian ? sizeof(word_type) * 8 - (pos + len) * kBaseSize : pos * kBaseSize;
  }
 ...
 public:
  ...
  explicit CompactVector(std::vector<word_type> *v, size_type size = 0) : size_(size), underlying_vec_(0), vec_ptr_(v) {
    resize(size);
  }
  ...
  Adapter operator[](size_type i) { return Adapter(vec_ptr_->data() + i / kBasesPerWord, i % kBasesPerWord); }
  ...
  void resize(size_type size) {
    ...
    if (size_ < old_size && size_ > 0 && size_ % kBasesPerWord != 0) {
      vec_ptr_->back() = sub_word(vec_ptr_->back(), 0, size_ % kBasesPerWord) << bit_shift(0, size_ % kBasesPerWord);
    }
  }
  ...
  void push_word(word_type val, unsigned pos = 0) { push_word(val, pos, kBasesPerWord - pos); }
  void pop_back() {
    (*this)[size_ - 1] = 0;
    --size_;
  }

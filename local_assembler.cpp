...
void LocalAssembler::ReadContigs(const char *fastx_file_name) {
	gzFile fp = strcmp(file_name, "-") == 0 ? gzdopen(file_no(stdin), "r") : gzopen(file_name, "r");
	assert(fp != NULL);
	if (contigs_ == NULL) {
	...
	if (file_type == kBinary) {
		...
		while (gzread(fp, &len, sizeof(uint32_t)) != 0) {
			...
			if (num_words > seq_capacity) {
				...
				seq = (uint32_t*) ReAllocAndCheck(sizeof(uint32_t) * seq_capacity, __FILE__, __LINE__);
			}
